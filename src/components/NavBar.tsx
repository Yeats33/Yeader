...
const NAV_ITEMS: NavItem[] = [
  {
    path: "/sources",
    label: "源",
    icon: `<img src="/logo.png" width="22" height="22" style="opacity:0.6" alt="源" />`,
    activeIcon: `<img src="/logo.png" width="22" height="22" alt="源" />`,
  },
  {
    path: "/feed",
    label: "订阅",
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg>`,
    activeIcon: `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg>`,
  },
  {
    path: "/settings",
    label: "设置",
...
export function NavBar({ routePath }: NavBarProps) {
  return (
    <nav className="bottom-nav" id="bottom-nav">
      {NAV_ITEMS.map((item) => {
        const isActive =
          routePath === item.path || (item.path !== "/" && routePath.startsWith(item.path));
        return (
          <button
            key={item.path}
